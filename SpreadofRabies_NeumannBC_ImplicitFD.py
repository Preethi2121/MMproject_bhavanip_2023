# -*- coding: utf-8 -*-
"""
Created on Tue Jan 17 06:30:44 2023
...
# Looping over time and space with combine explicit and implicits FD scheme
for n in range(0, Nt-1):
    # Create an array of the current I values
    I_current = I[:,n]
    # Compute the next S values using the explicit Euler method
    S[:,n+1] = S[:,n] - dt*r*S[:,n]*I_current
    # Compute the next I values using the implicit backward Euler method
    I_next = np.linalg.solve(np.eye(Nx) - dt*(r*S[:,n+1] - d), I_current + dt*D*(np.roll(I_current, -1) + np.roll(I_current, 1) - 2*I_current)/dx**2)
    I[:,n+1] = I_next
    # Neumann boundary conditions
    S[0,n+1] = 0 # derivative of S at the left boundary is zero
    S[Nx-1,n+1] = S[Nx-2,n+1] # derivative of S at the right boundary 
    I[0,n+1] = 0 # derivative of I at the left boundary is zero
    I[Nx-1,n+1] = I[Nx-1,n] + dt*r*S[Nx-1,n]*I[Nx-1,n] - dt*d*I[Nx-1,n] + dt*D*(I[0,n] + I[Nx-2,n] - 2*I[Nx-1,n])/dx**2 # derivative of I at the right boundary
...
plt.xlabel('Space')
plt.title('2D plot of Population density vs Space with Neumann boundary conditions & Implicit backward FD scheme')
...
plt.ylabel('Population density')
plt.xlabel('Time')
plt.title('2D plot of Population density vs Time with Neumann boundary conditions & Implicit backward FD scheme')
plt.text(80, 0.8*S.max(), 'Susceptible', fontsize=10, color='blue')
plt.text(80, 0.8*I.max(), 'Infected', fontsize=10, color='red')
plt.show()
plt.text
...
ax2.set_title('3D plot of Infected population in time and space with Implicit backward FD scheme')
plt.show()