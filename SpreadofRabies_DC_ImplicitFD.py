# -*- coding: utf-8 -*-
"""
Created on Thu Jan 19 10:30:05 2023
...
# Looping over time and space with Crack Nicholson that combine explicit and implicits FD scheme
...
plt.xlabel('Space')
plt.title('2D plot of Population density vs Spcae with DC boundary conditions & Implicit backward FD scheme')
...
plt.ylabel('Population density')
plt.xlabel('Time')
plt.title('2D plot of Population density vs Time with DC boundary conditions & Implicit FD scheme')
plt.text(80, 0.8*S.max(), 'Susceptible', fontsize=10, color='blue')
plt.text(80, 0.8*I.max(), 'Infected',fontsize=10, color='red')
plt.show()
plt.text
